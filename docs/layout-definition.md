...
| `navigationDrawer` | `[NavigationDrawerConfig](#navigationdrawerconfig)` | (Optional) An object that customizes the behavior of the navigation drawer.          |
...
- **`over`** – The drawer slides over the page content without pushing it. Closes when the user clicks outside.
- **`push`** – The drawer pushes the page content to the side when opened.
- **`side`** – The drawer stays permanently open alongside the page content.
