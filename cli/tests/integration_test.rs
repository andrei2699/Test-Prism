...
#[parameterized(input_file = {
    "tests/data/jest/input/jest-report.json",
}, expected_output_file = {
    "tests/data/jest/output/jest-report.json",
})]
fn jest_parse_command(input_file: &str, expected_output_file: &str) {
    let temp_dir = tempdir().expect("Failed to create temp dir");
    let output_path = temp_dir.path().join("output.json");
    let output_path_str = output_path.to_str().unwrap().to_string();
...
#[parameterized(input_file = {
    "tests/data/generic/input/generic-report.json",
}, expected_output_file = {
    "tests/data/generic/output/generic-report.json",
})]
fn generic_parse_command(input_file: &str, expected_output_file: &str) {
    let temp_dir = tempdir().expect("Failed to create temp dir");
    let output_path = temp_dir.path().join("output.json");
    let output_path_str = output_path.to_str().unwrap().to_string();
    ...
    parse_command(
        ...
        "2025-01-06T15:34:21.123Z".to_string(),
        vec![],
        Some("tests/data/generic/input/mapping.json".to_string()),
    );
