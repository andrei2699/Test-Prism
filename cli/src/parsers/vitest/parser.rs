use crate::parsers::vitest::models::VitestReport;
...
impl TestParser for VitestParser {
    fn parse(&self, file_path: &Path) -> Result<Vec<TestSuite>, String> {
        ...
        let report: VitestReport = serde_json::from_str(&content).map_err(|e| e.to_string())?;
                ...
                let tests = file_result
                    ...
                    .map(|assertion| {
                        ...
                        Test {
                            ...
                            time: assertion.duration.unwrap_or(0.0) / 1000.0,
                            status,
...
mod tests {
    ...
    fn parse_valid_vitest_json() {
        ...
        let parser = VitestParser;
        let result = parser.parse(file.path()).unwrap();

        assert_eq!(result.len(), 1);
        let suite = &result[0];
        assert_eq!(suite.name, "src/example.test.ts");
        assert_eq!(suite.file, Some("src/example.test.ts".to_string()));
        ...
        assert_eq!(
            ...
            TestStatus::Failed("expected 3 to be 4".to_string())
        );
